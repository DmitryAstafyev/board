...
export function validateEventDesc(
    desc: IEventDesc,
    target: any
): Error | undefined {
    ...
    if (desc.self instanceof Array) {
        ...
        return valid
            ? undefined
            : new Error(
                  `Self should be one of [${desc.self.join(
                      ", "
                  )}], but not one is suitable.`
              );
    }
    ...
    if (desc.self === null && target !== null && target !== undefined) {
        return new Error(
            `Self is defined as null, but target isn't null or undefined`
        );
    }
    if (desc.self !== null && (target === null || target === undefined)) {
        return new Error(
    ...
    if (typeof desc.self !== "string") {
        if (!(target instanceof desc.self)) {
            return new Error(
                `Expecting ${desc.self}, but has been gotten ${target}`
            );
        } else {
            return undefined;
        }
    ...
    if (desc.self !== typeof target) {
        return new Error(
            `Expecting target to be ${
                desc.self
            }, but target is ${typeof target}`
        );
    }
    ...
    required.forEach((name: string) => {
        const types: any[] =
            desc[name] instanceof Array ? desc[name] : [desc[name]];
        let valid: boolean = false;
...
export class Subject<T> {
    ...
    public static unsubscribe(subjects: unknown): void {
        ...
        Object.keys(subjects as object).forEach((key: string) => {
            const target = (subjects as { [key: string]: Subject<unknown> })[
                key
            ];
